...
FLUXIONScannerModeQuery="Select scanner enhancement mode"
FLUXIONScannerOptionStandard="Standard Scan (WPA/WPA2/WPA3)"
FLUXIONScannerOptionHighPower="High-Power Boost Scan (Max TxPower + All Bands)"
FLUXIONScannerOptionAllEnc="All Networks Scan (WPA + Open + WEP)"
FLUXIONScannerOptionDeep="Deep Scan (Extended Duration for Distant APs)"
FLUXIONTxPowerBoostingNotice="Boosting Wi-Fi adapter TxPower to maximum (30dBm / 1000mW)..."
FLUXIONSignalStrong="Strong"
FLUXIONSignalMedium="Medium"
FLUXIONSignalWeak="Weak"
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
